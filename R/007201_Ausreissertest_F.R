...
for (i in 1:length(dataset$x)) {
  ...
  outlier_f <- rbind(outlier_f, data.frame(i, Ausreisser_RS, Ausreisser_TS, Syx_1_cor))
}
...
for (i in 1:length(dataset$x)) {
  ...
  outlier_t <- rbind(outlier_t, data.frame(i, unten=y_cor-CI,oben= y_cor+CI, x=dataset$y[i]))
}